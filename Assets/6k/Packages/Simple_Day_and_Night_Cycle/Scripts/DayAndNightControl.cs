...
public class DayAndNightControl : MonoBehaviour {
	...
	private bool aDayPassed = false;
	private bool disablePause;
	private CreationEvents creationEvent;
	private bool newDay;
	public bool changeToMorning = false;
	[HideInInspector]
	...
	void Start () {
		creationEvent = FindObjectOfType<CreationEvents>();
        newDay = true;
		disablePause = true;
	...
	void Update () {
		...
		if (currentTime >= 0.3 && newDay)
		{
            ...
            if (aDayPassed)
			{
                StartCoroutine(creationEvent.StartDay());
			}
            newDay = false;
			changeToMorning = false;
		}
